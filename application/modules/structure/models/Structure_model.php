...
class Structure_model extends CI_Model
{
	...
	var $order = array('structureCode' => 'asc'); // default order 
	...
	{
		$this->db->from($this->table);
		$this->db->where('structureCode', $id)->where('deleteAt', NULL);
		$query = $this->db->get();
	...
	{
		$this->db->from($this->table);
		$this->db->where('slug', $slug)->where('deleteAt', NULL)->where('structureParent !=', '0');
		$query = $this->db->get();
		...
		$check = $this->db->get_where($this->table,[
			'structureParent' => $data['structureCode']
		])->row_array();
		if($check == NULL){
			return [
				'status' => TRUE,
	...
	public function delete_by_id($id)
	{
		...
		$this->db->update($this->table, $data, ['structureCode' => $id]);
		return $this->db->affected_rows();
	}
}
