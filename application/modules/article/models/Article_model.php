...
class Article_model extends CI_Model
{
	...
	var $table = 'article';
	var $column_order = array('title', 'content'); //set column field database for datatable orderable
	...
	private function _get_datatables_query()
	{
		...
		$this->db->from($this->table)->join('category', 'category.categoryCode=article.categoryCode')->where($this->table.'.deleteAt', NULL);
	...
	{
		$this->db->from($this->table);
		$this->db->where('articleCode', $id)->where('deleteAt', NULL);
		$query = $this->db->get();
	...
	public function get_by_slug($slug)
	{
		...
		$this->db->where('slug', $slug)->where('article.deleteAt', NULL);
		$query = $this->db->get();
	...
	public function delete_by_id($id)
	{
		...
		$this->db->update($this->table, $data, ['articleCode' => $id]);
		return $this->db->affected_rows();
	}
}
